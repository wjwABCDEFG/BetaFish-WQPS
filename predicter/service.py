...
def train_model(model_id, loop=3):
    ...
    return model_tester.test_model(model, test_set, mean_and_std, lstm_mode,spark_mode)
...
def train_vote_model(model_id, model_info):
    ...
    print("res_train_vote_model",res)
    return res


def predict_next_month(model_id, data, Month_days_num ):
    model_info = model_handler.get_model_info(model_id)
    obj = model_info["obj"]
    method = model_info["method"]
    lstm_mode = False
    ...
    raw_data = data_processor.load_all_data()
    mean_and_std = data_processor.get_mean_and_std(raw_data, obj)
    x = data_processor.standardize_for_prediction(mean_and_std, np.array(data), Month_days_num)
