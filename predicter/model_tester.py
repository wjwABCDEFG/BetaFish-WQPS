...
def test_model(model, test_set, mean_and_std, lstm_mode,spark_mode):
    col_num = np.size(test_set, 1)
    x = test_set[:, 0:col_num - 1]
    y = test_set[:, col_num - 1] * mean_and_std['std'] + mean_and_std['mean']
    pred = model_predicter.predict(model, x, mean_and_std, lstm_mode,spark_mode)
